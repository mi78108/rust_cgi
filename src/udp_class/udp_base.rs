...
pub static CLIENTS: LazyLock<RwLock<HashMap<String, Client>>> =
...
impl Client {
    ...
    pub fn from_str(data: &str, from: &str) -> Option<Self> {
        ...
        if rst.len() >= 2 {
            let via = Client::from_str(rst[2..].join("|").as_str(), from)
                .and_then(|v| Option::Some(Box::new(v)));
            return Some(Client {
                from: from.to_string(),
                addr: SocketAddr::from_str(rst.get(0).unwrap()).unwrap(),
                name: rst.get(1).unwrap().to_string(),
                via,
            });
        }
        return None;
        ...
        spawn(move || {
            ...
            script
                ...
                .envs(HashMap::from([
                    ...
                    ("Req_Peer_Ip".into(), format!("{}", addr.ip())),
                    ("Req_Peer_Port".into(), format!("{}", addr.port())),
                    ("req_body_method".into(), "UDP".into()),
                    ("req_script_path".into(), "/udp_handle".into()),
                ]))
                .stdin(Stdio::piped())
                .stdout(Stdio::piped());
