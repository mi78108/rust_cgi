                    ...
                    Ok(mut child) => {
                        ...
                        if let Some(mut stdin) = script_stdin {
                            ...
                            while let Ok(len) = _req_stdin.read_from(&mut buffer) {
                                ...
                                recv_len += len;
                                if len > 0 {
                                    debug!("script stdin write [{}] [{}]",len,String::from_utf8_lossy(&buffer[..len]));
                                    ...
                                    if let Some(content_length) = _req_stdin.headers.get("Content-Length") {
                        ...
                        unsafe {
                            ...
                            if let Err(_e) = child.kill() {
                                ...
                                if libc::kill(child.id() as i32,libc::SIGTERM) == 0 {
                                    ...
                                    error!("script stdout thread kill erro by sigterm {:?}",std::io::Error::last_os_error())
                                }
                            } else {
                                debug!("script stdout thread kill done")
                            }
