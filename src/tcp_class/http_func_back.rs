...
impl Req for Websocket {
    fn read(&self, data: &mut Vec<u8>) -> Result<Option<usize>, std::io::Error> {
        ...
        if let Err(e) = reader.read_exact(&mut bytes) {
            error!("[{:?}] websocket read package length fail read bytes part1 {:?}",
                tid,e
            );
            return Err(e);
        }
        debug!("[{:?}] websocket read byte 1:{:b}",tid, bytes[0]);
        debug!("[{:?}] websocket read byte 2:{:b}",tid, bytes[1]);
        ...
        if let Err(e) = length_rst {
            error!("[{:?}] websocket read package length fail read bytes part2  {:?}",
                tid,e
            );
            return Err(e);
        }
        ...
        if let Err(e) = reader.read_exact(&mut mask) {
            error!("[{:?}]websocket read package length fail read bytes mask {:?}", tid,e);
            return Err(e);
        }
        //println!("get mask {:?}", _mask);
        //get play load
        debug!("[{:?}] websocket read package length {}",tid, length);
        data.resize(length, 0);
        if let Err(e) = reader.read_exact(data) {
            error!("[{:?}] websocket read package length fail read bytes:{} {:?}",
                tid,length, e
            );
            return Err(e);
        }
        ...
        match bytes[0] {
            ...
            h if h == 0b10001000 => {
                ...
                debug!("[{:?}] websocket ctrl close event",tid);
                Ok(None)
            }
            h if h == 0b10001001 => {
                // ctrl ping 0x9 0b10001001
                // ctrl pong 0xA 0b10001010
                debug!("[{:?}] websocket ctrl ping event", tid);
                self.write_with_h1(0b10001010, &[]).unwrap();
                Ok(Some(0))
            }
            _ => Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
