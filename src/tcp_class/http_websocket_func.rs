...
pub struct Websocket<'a,  'b> {
    base_on: &'a mut Http<'b>,
    processed: AtomicUsize,
    remainder: AtomicUsize,
    payload_mask: AtomicU32,
}

impl<'a, 'b> Websocket<'a, 'b> {
        ...
        if let Ok(_) = self.base_on.write(&[opcode]).await {
            ...
            match len {
                ...
                n if n >= 126 && n < (2usize).pow(16) => {
                    ...
                    // 2byte
                       let rst: Result<usize, Error> = self.base_on.write(&[(len >> 8) as u8, len as u8]).await;
                       rst.unwrap();
                    // &(*self.base_on)
...
impl<'a, 'b> Req for Websocket<'a, 'b> {
    async fn read(&self, data: &mut [u8]) -> Result<Option<usize>, Error> {
        ...
        if self.remainder.load(Ordering::Relaxed) > data.len() {
            let rst: Result<Option<usize>,Error> = self.base_on.read(data).await;
            rst.and_then(|len_opt| {
                if let Some(len) = len_opt {
            ...
            let rst: Result<Option<usize>,Error> = self.base_on.read(&mut buffer).await;
            rst.and_then(|len_opt| {
                if let Some(len) = len_opt {
...
impl<'a,'b> Handle<'a, Http<'b>> for Websocket<'a,'b> {
    // fn name() -> &'static str {
    //     "WEBSOCKET"
    // }

    async fn matches(stream: &Http<'b>) -> bool {
        if let Some(upgrade) = stream.env().get("upgrade") {
            let upgrade:&String = upgrade;
    ...
    async fn handle(stream: &'a mut Http<'b>) -> Result<Self, Error> {
